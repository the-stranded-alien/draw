...
import { ZoomIn, ZoomOut, Grid3X3, Sun, Moon, Undo2, Redo2, Trash2 } from "lucide-react";
...
export default function BottomBar() {
  const {
    appState, setZoom, setScroll, toggleTheme, toggleGrid,
    undo, redo, resetCanvas, historyIndex, history,
  } = useDrawStore();
  ...
  return (
    ...
    >
      {/* Undo / Redo */}
      <BarButton onClick={undo} title="Undo (⌘Z)" disabled={!canUndo}>
        <Undo2 size={16} strokeWidth={1.75} />
      </BarButton>
      <BarButton onClick={redo} title="Redo (⌘⇧Z)" disabled={!canRedo}>
      ...
      {/* Zoom */}
      <BarButton
        onClick={() => setZoom(Math.max(appState.zoom.value - ZOOM_STEP, MIN_ZOOM))}
        title="Zoom out (⌘−)"
      >
      ...
      <Tooltip>
        <TooltipTrigger>
          <button
            ...
            className={cn(
              "min-w-[54px] rounded-xl px-2 py-1",
      ...
      <BarButton
        onClick={() => setZoom(Math.min(appState.zoom.value + ZOOM_STEP, MAX_ZOOM))}
        title="Zoom in (⌘+)"
      >
      ...
      <BarButton onClick={toggleGrid} title={appState.showGrid ? "Hide grid" : "Show grid"}>
        <Grid3X3
          size={16}
          strokeWidth={1.75}
          className={appState.showGrid ? "text-[#6965db]" : ""}
        />
      </BarButton>
